...
events_file_path = os.path.join(project_path, "data/events.txt")
piriority_file_path = os.path.join(project_path, "data/priority_multipliers.txt")
rates_file_path = os.path.join(project_path, "data/telescope_rates.txt")

def _load_events(file_path: str = events_file_path) -> list:
    events = []
...
EVENTS = _load_events()
TELESCOPE_RATES = _load_rates(rates_file_path)
PRIORITY_MULTIPLIERS = _load_rates(piriority_file_path)
